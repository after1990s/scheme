...
; (newline)
 (cond ((self-evaluating? exp) exp)
    ...
    (make-procedure (lambda-parameters exp) (lambda-body exp) env))
     ...
     ((let*? exp) (user-eval  (let*->nested-lets exp) env))
...
;exercise 4.7
(define (let*? exp) (tagged-list? exp 'let*))
;I assume that we have a stk lib.
(load "stack.scm")
(define (let*->nested-lets exp)
(let ((stk (make-stack)))
       (set-car! exp 'let)
       (let*-push-vars stk (cadr exp))
       ;(display (cadr exp))
       (let*-pop-and-rebuild-vars stk (cdr exp))
       (let*-pop-and-rebuild-remain-vars stk (cadr exp))
       (display "let*->netsted-lets DEBUG INFO, exp:")
       (display exp)
       (newline)
       exp
       ))

(define (let*-push-vars stk  exp)
       (display "let*->push-vars DEBUG INFO, exp:")
       (display exp)
       (newline)
  (if (not(eq? exp '()))
      (begin (push stk (car exp))
             (let*-push-vars stk (cdr exp)))))

(define (let*-pop-and-rebuild-vars stk exp)
       (display "let*->pop-and-rebuild-vars DEBUG INFO, exp:")
       (display exp)
       (newline)
       (display-stack stk)
  (if (not (empty-stack? stk))
      (set-car! exp (list (pop stk)))))

(define (let*-pop-and-rebuild-remain-vars stk exp)
       (display "let*->pop-and-rebuild-remain-vars DEBUG INFO, exp:")
       (display exp)
       (newline)
  (if (not (empty-stack? stk))
      (begin (set-cdr! exp (list (pop stk)))
             (let*-pop-and-rebuild-remain-vars stk (cdr exp)))))
;end of exercise 4.7
