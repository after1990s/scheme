...
(define (user-eval exp env)
 (analyze exp) env)
(define (analyze exp)
 (cond ((self-evaluating? exp) (analyze-self-evaluation exp))
     ((bool? exp) (analyze-bool exp))
     ((variable? exp) (analyze-variable-value exp))
     ((quoted? exp) (analyze-quoted exp))
     ((assignment? exp) (analyze-assignment exp))
     ((definition? exp) (analyze-definition exp))
     ((if? exp) (analyze-if exp))
     ((lambda? exp) (analyze-lambda exp))
     ((begin? exp) (analyze-sequence (begin-actions exp) ));begin statment
     ((cond? exp) (analyze (cond->if exp) ))
     ((let? exp) (analyze (let->combination exp ) (let->arguments exp) ));not done
